...
**Evidence / large-artifact storage (strategy):** Git retains scenario definitions, manifests, checksums, and compact reference corpora/baselines. Large corpora/result bundles, when needed, are referenced by content identity (`sha256` + `sizeBytes`) with optional archive locators (public default: GitHub Release assets on evidence checkpoint tags). Location is not identity; local verification is `scripts/verify-evidence-artifact.sh`. See [`evaluation/EVIDENCE_ARTIFACT_STORAGE.md`](evaluation/EVIDENCE_ARTIFACT_STORAGE.md). No Evaluation Kit storage service, cloud SDK, or Maven module is introduced for corpora.

Offline evaluation and candidate-model tooling (not on the request path):

- **Detection Evaluation Framework** — reference dataset → deterministic replay → metrics/evidence (`DetectionEvaluationRunner`). Details: [`evaluation/DETECTION_EVALUATION.md`](evaluation/DETECTION_EVALUATION.md).
- **Evaluation Kit surface** — generated-corpus and BYO dataset evaluation reuse that offline path inside **`ai-sentinel-core`** (CLI: `scripts/evaluate-generated-corpus.sh`; optional `Dockerfile.evaluation-kit`). There is **no** separate Evaluation Kit Maven module; `ai-sentinel-benchmark` remains JMH-only.
- **Candidate lifecycle (0.4.0 engineering capability)** — optional offline load → evaluate → accept → observational shadow → designation-only promotion. Shadow never drives policy (`SHADOW RESULT != PRODUCTION DECISION`). Promotion does not rewire the starter (`PROMOTED LIFECYCLE CHAMPION != RUNNING PRODUCTION SCORER`). Contracts: [`docs/contracts/SCORER_ARTIFACT.md`](docs/contracts/SCORER_ARTIFACT.md) and sibling candidate docs.
- **Official Detection Reference Baseline** — capture / verify / lifecycle under [`evaluation/DETECTION_REFERENCE_BASELINE.md`](evaluation/DETECTION_REFERENCE_BASELINE.md).

None of the above establishes production efficacy (`FRAMEWORK ACCEPTANCE != DETECTION QUALITY ACCEPTANCE`, `DETECTION BASELINE != PRODUCTION EFFICACY`, `ACCEPTANCE != AUTOMATIC SHADOW ENABLEMENT`, `PROMOTION != PRODUCTION DEPLOYMENT`).
...
**Distributed trust baselines** (optional): Redis keys are `{prefix}{sha256(logicalKey)}` with TTL refresh on write and **no application-side Redis cardinality cap** (`baseline-max-keys` applies only to the in-memory store / fail-open fallback). Operators must size Redis for peak unique identities within TTL — see `docs/configuration.md`. On Redis outage, trust falls back to **per-instance** in-memory state; fallback observations are **not** auto-reconciled into Redis after recovery.

Architecture statement: Redis-backed shared quarantine / throttle / trust state is **supported**. What repository tests prove (same-version multi-client consistency vs one Redis backend) lives in [`docs/deployment.md`](docs/deployment.md#distributed-deployment-notes) — that evidence is not Redis Cluster, Kubernetes, rolling-deploy, or mixed-version proof.
...
- **Distributed / Redis** — `dev.aisentinel.validation.*` and related tests: Testcontainers Redis and/or ephemeral Docker CLI `redis:7-alpine` (skipped when Docker cannot start Redis). These suites prove **single-JVM** coordination through Redis (often with a second Lettuce client as a stand-in peer), including same-version multi-client write/read consistency for quarantine, identity trust baselines, and cluster throttle counters. They are **not** a multi-process / multi-host / mixed-version / rolling-deploy proof — see [`docs/deployment.md`](docs/deployment.md) distributed notes.
