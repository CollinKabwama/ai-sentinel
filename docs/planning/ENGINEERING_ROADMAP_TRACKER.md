...
| ENG-008 | Reference synthetic / evaluation dataset | DONE | ENG-021 | `evaluation/reference/manifest.json`; `evaluation/reference/events.jsonl`; `evaluation/reference/annotations.json`; `evaluation/REFERENCE_DATASET.md`; `ReferenceDatasetGenerator.java`; `ReferenceDatasetValidator.java`; `ReferenceDatasetGeneratorTest.java`; `scripts/generate-reference-dataset.sh` | Preserve dataset determinism and extend only with backward-compatible annotation/generation rules | No |
...
- deterministic evaluation-observation alignment between reference annotations and replay results;
- explicit evaluable-event ordering and scenario attribution contracts for later metrics.
...
- `ai-sentinel-core/src/main/java/dev/aisentinel/core/evaluation/EvaluationObservation.java`
- `ai-sentinel-core/src/test/java/dev/aisentinel/core/evaluation/ReferenceEvaluationAlignerTest.java`
- `evaluation/DETECTION_EVALUATION.md`
- `docs/performance/BENCHMARKING.md`
...
- provide the first tracked deterministic, privacy-safe synthetic evaluation corpus for future replay and detection work.
...
- tracked repository-level dataset in `evaluation/reference/` with `manifest.json`, `events.jsonl`, and versioned `annotations.json`;
- deterministic generator using the production `DefaultFeatureExtractor` with an injected synthetic clock;
- deterministic manifest timestamp, deterministic event/correlation IDs, and append-order preservation through `EvaluationDatasetWriter`;
- bounded behavioral scenario corpus covering warmup, baseline formation, abrupt deviation, gradual change, legitimate anomalous behavior, and interleaved identities;
- validation and regeneration proof via `ReferenceDatasetValidator`, focused tests, and `scripts/generate-reference-dataset.sh`.

Important retained invariants:

- synthetic labels live only in annotations
- anomalous is not equivalent to malicious
- scorer output is not ground truth
- reference dataset is not the detection baseline
