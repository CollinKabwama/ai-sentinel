...
class AsyncTrainingCandidatePublisherTest {
    ...
    private static final class CapturingTransport implements TrainingCandidateTransport {
        final List<dev.aisentinel.distributed.training.TrainingCandidateRecord> records = new ArrayList<>();

        @Override
        public void send(dev.aisentinel.distributed.training.TrainingCandidateRecord record) {
            records.add(record);
        }
    }
}
