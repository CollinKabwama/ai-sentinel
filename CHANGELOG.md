...
- Candidate scorer/model **artifact contract and validation** (`ScorerArtifactDescriptor`, artifact digest / configuration fingerprint checks). Valid artifact metadata is not a good model and not production authority. See [`docs/contracts/SCORER_ARTIFACT.md`](docs/contracts/SCORER_ARTIFACT.md).
- Candidate scorer **loading, health, and runtime isolation** (`CandidateScorerLoader`): bounded SHA-256-verified Isolation Forest/`aif1` load into operational readiness. Verified bytes are the consumed bytes; readiness is not quality, acceptance, or production authority. See [`docs/contracts/SCORER_CANDIDATE_LOADING.md`](docs/contracts/SCORER_CANDIDATE_LOADING.md).
