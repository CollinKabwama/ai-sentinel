...
- **Fail-open** — Many optional paths (e.g. distributed Redis, async training publish) are designed so failures degrade to permissive behavior where documented, to avoid accidental total outage. This trades strict lockdown for availability; operators must tune flags and monitor metrics. Prefer **`MONITOR`** until fail-open rates and enforcement decisions are understood in your traffic — see [`docs/deployment.md`](docs/deployment.md).
